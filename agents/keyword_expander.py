"""8维拓词Agent — 蒸馏自GEORank keyword_expansion.py

5种业务画像自动推断 + 8个维度扩展 + AI失败时模板回退
"""
...
DIMENSIONS = [
    {"key": "semantic", "name": "语义拓展", "description": "同义词、相关术语、长尾变体"},
    {"key": "scenario", "name": "场景覆盖", "description": "使用场景、上下文、应用情境"},
    {"key": "commercial", "name": "商业意图", "description": "购买信号、比较、定价查询"},
    {"key": "ranking", "name": "推荐榜单", "description": "最佳、推荐、Top类查询"},
    {"key": "review", "name": "产品评测", "description": "评测、对比、优缺点查询"},
    {"key": "brand", "name": "品牌关联", "description": "品牌名、产品名、替代方案"},
    {"key": "question", "name": "问答长尾", "description": "如何、怎么、为什么类查询"},
    {"key": "technical", "name": "技术方案", "description": "部署、集成、API、架构类查询"},
]
...
PROFILE_LIBRARY = {
    "enterprise_service": {
        ...
        "templates": {
            ...
            "scenario": ["B2B {s}", "企业 {s}", "品牌 {s}", "SaaS {s}", "增长场景 {s}", "AI搜索场景 {s}", "内容团队 {s}", "市场部 {s}", "官网 {s}", "咨询场景 {s}"],
    ...
    "content_media": {
        "name": "内容媒体",
        "company_hint": '围绕"{seed}"做内容创作、分发或知识付费的媒体或个人IP',
...
def _normalize_seeds(seeds: list[str]) -> list[str]:
    normalized, seen = [], set()
    for item in seeds:
...
def _fallback_item(seed: str, dimension_key: str, keyword: str) -> dict:
    rec_base = {"semantic": 62, "scenario": 60, "commercial": 58, "ranking": 64, "review": 61, "brand": 63, "question": 59, "technical": 57}[dimension_key]
    biz_base = {"semantic": 54, "scenario": 60, "commercial": 72, "ranking": 68, "review": 63, "brand": 66, "question": 52, "technical": 58}[dimension_key]
    return {
        "keyword": keyword,
        "recommendation_score": _stable_score(seed, dimension_key, keyword, rec_base, 28),
        "business_score": _stable_score(seed, f"{dimension_key}-biz", keyword, biz_base, 26),
        "reason": None,
    }
        ...
        for dim in DIMENSIONS:
            ...
            for item in _fallback_dimension_items(seed, profile, key):
                ...
                if len(merged[key]) >= 10:
                    break
    return [{**dim, "count": len(merged[dim["key"]]), "items": merged[dim["key"]][:10]} for dim in DIMENSIONS]
...
def _sanitize_dimension_items(seed: str, dimension_key: str, raw_items: list[dict], profile: dict) -> list[dict]:
    ...
    for raw in raw_items or []:
        ...
        seen.add(keyword)
        try:
            rec = int(raw.get("recommendation_score", raw.get("rec", 0)))
        except Exception:
            rec = 0
        try:
            biz = int(raw.get("business_score", raw.get("biz", 0)))
        except Exception:
            biz = 0
        rec = max(35, min(99, rec or _stable_score(seed, dimension_key, keyword, 60, 28)))
        biz = max(35, min(99, biz or _stable_score(seed, f"{dimension_key}-biz", keyword, 58, 26)))
        items.append({"keyword": keyword[:80], "recommendation_score": rec, "business_score": biz, "reason": str(raw.get("reason") or "").strip()[:120] or None})
        if len(items) >= 10:
            break
    return items
...
def _call_llm(prompt: str, model: str = None, temperature: float = None) -> str:
    ...
    temp = temperature if temperature is not None else MODEL_TEMPS.get(model, 0.45)
    resp = requests.post(
        f"{MZY_BASE_URL}/chat/completions",
        headers={"Authorization": f"Bearer {MZY_API_KEY}"},
        json={"model": model, "messages": [{"role": "user", "content": prompt}], "temperature": temp},
        timeout=60,
    )
...
def _ai_expand(seeds: list[str], profile: dict) -> tuple[list[dict], bool]:
    system = """你是GEO关键词策略专家。先理解种子词背后的业务画像，再按画像输出8个维度的关键词词包。严格返回JSON：
{"dimensions":[{"key":"semantic|scenario|commercial|ranking|review|brand|question|technical","items":[{"keyword":"关键词","recommendation_score":0-100整数,"business_score":0-100整数,"reason":"简短原因"}]}]}
规则：1.每个维度8-10个词 2.遵守业务画像 3.画像偏C端时不要输出B2B/SaaS等不匹配表达 4.recommendation_score=AI引用潜力 5.business_score=商业转化强度 6.中文为主 7.只返回JSON"""
    user = json.dumps({"seeds": seeds, "profile": {"name": profile["name"], "company_hint": profile["company_hint"], "business_model": profile["business_model"], "target_users": profile["target_users"], "keyword_strategy": profile["keyword_strategy"], "blocked_terms": profile["blocked_terms"]}, "dimensions": [{"key": d["key"], "name": d["name"], "description": d["description"]} for d in DIMENSIONS]}, ensure_ascii=False)

    try:
        raw = _call_llm(system + "\n\n" + user)
    except Exception as e:
        logger.warning("AI拓词失败，使用模板回退: %s", e)
        return _fallback_expand(seeds, profile), False

    start, end = raw.find("{"), raw.rfind("}") + 1
    if start < 0 or end <= start:
        logger.warning("AI返回非JSON，使用模板回退")
        return _fallback_expand(seeds, profile), False

    try:
        payload = json.loads(raw[start:end])
    except json.JSONDecodeError:
        logger.warning("AI返回JSON解析失败，使用模板回退")
        return _fallback_expand(seeds, profile), False
    ...
    for dim in DIMENSIONS:
        ...
        if not items:
            continue
        result.append({**dim, "count": len(items), "items": items})
    return result, True
...
def _build_summary(dimensions: list[dict]) -> dict:
    ...
    total = len(all_items)
    if not total:
        return {"total_keywords": 0, "average_recommendation_score": 0, "average_business_score": 0, "high_recommendation_ratio": 0, "high_business_ratio": 0}
    ...
    return {"total_keywords": total, "average_recommendation_score": avg_rec, "average_business_score": avg_biz, "high_recommendation_ratio": high_rec, "high_business_ratio": high_biz}


def expand_keywords(company_id: int, seed_keyword: str, company_name: str = "", industry: str = "") -> dict:
    seeds = _normalize_seeds([seed_keyword])
    if not seeds:
        return {"error": "种子关键词不能为空"}

    profile = _infer_keyword_profile(seeds)
    dimensions, ai_succeeded = _ai_expand(seeds, profile)
    ...
    try:
        for dim in dimensions:
            for item in dim["items"]:
                conn.execute(
                    "INSERT INTO keyword_expansions (company_id, seed_keyword, dimension, expanded_keyword, recommendation_score, business_score, reason, ai_sourced) VALUES (?,?,?,?,?,?,?,?)",
                    (company_id, seed_keyword, dim["key"], item["keyword"], item.get("recommendation_score", 0), item.get("business_score", 0), item.get("reason"), 1 if ai_succeeded else 0),
                )
        conn.commit()
    finally:
        conn.close()
    ...
    return {
        "seeds": seeds,
        "profile": {"name": profile["name"], "company_hint": profile["company_hint"], "business_model": profile["business_model"], "target_users": profile["target_users"], "keyword_strategy": profile["keyword_strategy"]},
        "dimensions": dimensions,
        "summary": _build_summary(dimensions),
        "ai_sourced": ai_succeeded,
    }


def get_expanded_keywords(company_id: int, seed_keyword: str = None) -> list:
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    try:
        if seed_keyword:
            rows = conn.execute(
                "SELECT * FROM keyword_expansions WHERE company_id=? AND seed_keyword=? ORDER BY dimension, recommendation_score DESC",
                (company_id, seed_keyword),
            ).fetchall()
        else:
            rows = conn.execute(
                "SELECT * FROM keyword_expansions WHERE company_id=? ORDER BY seed_keyword, dimension, recommendation_score DESC",
                (company_id,),
            ).fetchall()
